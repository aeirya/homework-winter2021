...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Lock ()
          var intState : int = SetInterruptsTo (DISABLED)
          if count == 0x7fffffff
            FatalError ("Mutex count overflowed during 'Lock' operation")
          endIf
          if holder
              if holder.name == currentThread.name
                FatalError ("Thread already holds the mutex")
              endIf
              --  print ("going to sleep... \n")
              -- since another thread holds the mutex, will go to sleep untill interrupted
              waitingThreads.AddToEnd (currentThread)
              count = count + 1
              currentThread.Sleep ()
          else
              --  print ("aquired lock \n")
              -- if there's no thread holding the mutex, it is safe to pick
              holder = currentThread
          endIf
          intState = SetInterruptsTo (intState)
        endMethod
      ...
      method Unlock ()
          var
            intState: int
            t: ptr to Thread
          -- only unlock the mutex if the calling thread is allowed to!
          if self.IsHeldByCurrentThread ()
            intState = SetInterruptsTo (DISABLED)
            holder = null
            -- awaken another thread waiting to gain access
            if count > 0
              count = count - 1
              t = waitingThreads.Remove ()
              t.status = READY
            ...
            FatalError ("You cannot unlock the mutex you do not hold")
          endIf
        endMethod
      ...
      method IsHeldByCurrentThread () returns bool
          if currentThread.name == holder.name
            return true
          endIf
          return false
        endMethod
